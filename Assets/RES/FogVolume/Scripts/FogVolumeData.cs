   ...
   public void FindFogVolumes()
    ...
    void Update()
    {
        ...
        for (int i = 0; i < SceneFogVolumes.Length; i++)
        {
            ...
            if (SceneFogVolumes.Length==0) DestroyImmediate(gameObject);
#endif
    }
    void ToggleFogVolumeRenderers()
    {
        if (FoundCameras != null)
            for (int i = 0; i < FoundCameras.Count; i++)
            {
                ...
                {
                    var renderer = FoundCameras[i].GetComponent<FogVolumeRenderer>();
                    if (renderer == null)
                    {
                        if (ForceNoRenderer) { continue; }
                        renderer = FoundCameras[i].gameObject.AddComponent<FogVolumeRenderer>();
                    }

                    renderer.enabled = ForceNoRenderer ? false : true;
                }
            }
    }

...
public class FogVolumeDirectionalLight : MonoBehaviour
{
    ...
    void OnEnable()
    {
        _GO_Canvas = GameObject.Find("Debug Canvas");
        if (!_GO_Canvas) _GO_Canvas = new GameObject("Debug Canvas");
        _GO_Image = GameObject.Find("Image");
        if (!_GO_Image)
        {
            _GO_Image = new GameObject("Image");
